...
from data_imput import output_data
...
def simple_iteration_method(system, method):
    ...
    for i in range(max_iter):
        ...
        if (abs(x - x0) < inaccuracy) and (abs(y - y0) < inaccuracy):
            ...
            print(f"Погрешность x = {x - f1(x, y, system)}\nПогрешность y = {y - f2(x, y, system)}")
            break
        x0, y0 = x, y
        iteration += 1
...
def f1(x, y, system):
    if system == 1:
        ...
        print("Система вне списка выбора")
        exit()
