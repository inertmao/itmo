...
def quation_solution(quation, x):
    if quation == 1:
        return np.float64(x**2 - 3*x + 2)
    elif quation == 2:
        return np.float64(x**3 + 2*(x**2) - 5)
    elif quation == 3:
        return np.float64(np.cos(x) - x**2)
    else:
        print("Неизвестное уравнение")
        exit()

def quation_df_solution(quation, x):
    if quation == 1:
        return np.float64(2*x - 3)
    elif quation == 2:
        return np.float64(3*(x**2) + 4*x)
    elif quation == 3:
        return np.float64(-2*x - sin(x))
    else:
        print("Неизвестное уравнение")
        exit()
...
def validate_roots(num_roots):
    ...
    if num_roots < 1:
        ...
        print("Больше одного корня (", num_roots, ")")
        return False
    return True